...
for meg,mri in sub_dict.items():
    ...
    # load CSD matrix for each block and conditions (diff files for diff freq bands*!)
    csd_rest = mne.time_frequency.read_csd("{dir}nc_{meg}-csd_rest_beta_low.h5".format(dir=meg_dir,meg=meg))
    csd_ton = mne.time_frequency.read_csd("{dir}nc_{meg}-csd_ton_beta_low.h5".format(dir=meg_dir,meg=meg))
    csd_a = mne.time_frequency.read_csd("{dir}nc_{meg}-csd_a_beta_low.h5".format(dir=meg_dir,meg=meg))
    csd_b = mne.time_frequency.read_csd("{dir}nc_{meg}-csd_b_beta_low.h5".format(dir=meg_dir,meg=meg))
    csd_pos_a = mne.time_frequency.read_csd("{dir}nc_{meg}-csd_pos_a_beta_low.h5".format(dir=meg_dir,meg=meg))
    csd_pos_b = mne.time_frequency.read_csd("{dir}nc_{meg}-csd_pos_b_beta_low.h5".format(dir=meg_dir,meg=meg))
    csd_neg_a = mne.time_frequency.read_csd("{dir}nc_{meg}-csd_neg_a_beta_low.h5".format(dir=meg_dir,meg=meg))
    csd_neg_b = mne.time_frequency.read_csd("{dir}nc_{meg}-csd_neg_b_beta_low.h5".format(dir=meg_dir,meg=meg))
    ...
    filters_rest = make_dics(rest.info,fwd_rest,csd_rest,pick_ori='max-power',rank='full',inversion='single',weight_norm="unit-noise-gain",normalize_fwd=False,real_filter=True)
    filters_ton = make_dics(ton.info,fwd_ton,csd_ton,pick_ori='max-power',rank='full',inversion='single',weight_norm="unit-noise-gain",normalize_fwd=False,real_filter=True)
    filters_a = make_dics(epo_a.info,fwd_a,csd_a,pick_ori='max-power',rank='full',inversion='single',weight_norm="unit-noise-gain",normalize_fwd=False,real_filter=True)
    filters_b = make_dics(epo_b.info,fwd_b,csd_b,pick_ori='max-power',rank='full',inversion='single',weight_norm="unit-noise-gain",normalize_fwd=False,real_filter=True)
    ...
    stc_diff_rest = (stc_neg - stc_pos) / stc_rest
    stc_diff_ton = (stc_neg - stc_pos) / stc_ton

    # plot the difference between conditions
    stc_tonloc.plot(subjects_dir=mri_dir,subject=mri,hemi='both',time_viewer=True)
    stc_diff_rest.plot(subjects_dir=mri_dir,subject=mri,hemi='both',time_viewer=True)
    stc_diff_ton.plot(subjects_dir=mri_dir,subject=mri,hemi='both',time_viewer=True)

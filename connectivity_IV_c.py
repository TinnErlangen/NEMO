...
save_dir = "D:/NEMO_analyses/plots/connectivity_hcp/"   # for plots
...
for contrast in contrasts:
    ...
    for freq,vals in freqs.items():
        ...
        if not con_clust.n_connections == 0:
            ...
            labels = mne.read_labels_from_annot('fsaverage', 'HCPMMP1_combined',subjects_dir=mri_dir)
            del labels[:2]  # drop '???' labels
            con_parc_sum = con_clust.parcellate(labels, summary='sum',weight_by_degree=True)
            con_parc_sum.save('{dir}NEMO_{cont}_contrast_{f}-HCP-label-sum-connectivity.h5'.format(dir=meg_dir,cont=contrast,f=freq))
            # print them out
            ...
            brain.scale_data_colormap(fmin, fmid/4, fmax, True,center=0)   # optimize these values !!
            brain.add_annotation('HCPMMP1_combined', borders=1, alpha=0.9)

            # Save some views
            mlab.view(0, 90, 450, [0, 0, 0])
            mlab.savefig('{d}{cont}_connectivity_{f}_hcp_sum_rh.png'.format(d=save_dir,cont=contrast,f=freq), magnification=4)
            mlab.view(180, 90, 450, [0, 0, 0])
            mlab.savefig('{d}{cont}_connectivity_{f}_hcp_sum_lh.png'.format(d=save_dir,cont=contrast,f=freq), magnification=4)
            mlab.view(180, 0, 450, [0, 10, 0])
            mlab.savefig('{d}{cont}_connectivity_{f}_hcp_sum_top.png'.format(d=save_dir,cont=contrast,f=freq), magnification=4)
            mlab.view(180, 180, 480, [0, 10, 0])
            mlab.savefig('{d}{cont}_connectivity_{f}_hcp_sum_bottom.png'.format(d=save_dir,cont=contrast,f=freq), magnification=4)
            # mlab.close(fig)
            ...
            fig.savefig('{d}{cont}_connectivity_{f}_hcp_sum_squircle.pdf'.format(d=save_dir,cont=contrast,f=freq), bbox_inches='tight')

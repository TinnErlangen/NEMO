...
with open(logfile,"w") as file:
    ...
    for meg,mri in sub_dict.items():
        ...
        pos = epo_all['positive']
        neg = epo_all['negative']
        ...
        if len_diff < 0:
            ...
            for drop in drops:
                neg.drop([drop])
        # append to new epo_exp and save for further analyses
        epo_exp = mne.concatenate_epochs([ton,pos,neg])
        epo_exp.save("{dir}nc_{sub}_exp-epo.fif".format(dir=meg_dir,sub=meg))
        # # in case tones need to be differentiated: (leave for now)
        ...
        csd_neg_alpha = csd_morlet(neg, frequencies=freqs["alpha"], n_jobs=8, n_cycles=cycles["alpha"], decim=1)
        csd_neg_alpha.save("{dir}nc_{meg}-csd_neg_alpha.h5".format(dir=meg_dir,meg=meg))
        csd_neg_theta = csd_morlet(neg, frequencies=freqs["theta"], n_jobs=8, n_cycles=cycles["theta"], decim=1)
        csd_neg_theta.save("{dir}nc_{meg}-csd_neg_theta.h5".format(dir=meg_dir,meg=meg))
        csd_neg_beta_low = csd_morlet(neg, frequencies=freqs["beta_low"], n_jobs=8, n_cycles=cycles["beta_low"], decim=1)
        csd_neg_beta_low.save("{dir}nc_{meg}-csd_neg_beta_low.h5".format(dir=meg_dir,meg=meg))
        csd_neg_beta_high = csd_morlet(neg, frequencies=freqs["beta_high"], n_jobs=8, n_cycles=cycles["beta_high"], decim=1)
        csd_neg_beta_high.save("{dir}nc_{meg}-csd_neg_beta_high.h5".format(dir=meg_dir,meg=meg))
        csd_neg_gamma = csd_morlet(neg, frequencies=freqs["gamma"], n_jobs=8, n_cycles=cycles["gamma"], decim=1)
        csd_neg_gamma.save("{dir}nc_{meg}-csd_neg_gamma.h5".format(dir=meg_dir,meg=meg))

        csd_pos_alpha = csd_morlet(pos, frequencies=freqs["alpha"], n_jobs=8, n_cycles=cycles["alpha"], decim=1)
        csd_pos_alpha.save("{dir}nc_{meg}-csd_pos_alpha.h5".format(dir=meg_dir,meg=meg))
        csd_pos_theta = csd_morlet(pos, frequencies=freqs["theta"], n_jobs=8, n_cycles=cycles["theta"], decim=1)
        csd_pos_theta.save("{dir}nc_{meg}-csd_pos_theta.h5".format(dir=meg_dir,meg=meg))
        csd_pos_beta_low = csd_morlet(pos, frequencies=freqs["beta_low"], n_jobs=8, n_cycles=cycles["beta_low"], decim=1)
        csd_pos_beta_low.save("{dir}nc_{meg}-csd_pos_beta_low.h5".format(dir=meg_dir,meg=meg))
        csd_pos_beta_high = csd_morlet(pos, frequencies=freqs["beta_high"], n_jobs=8, n_cycles=cycles["beta_high"], decim=1)
        csd_pos_beta_high.save("{dir}nc_{meg}-csd_pos_beta_high.h5".format(dir=meg_dir,meg=meg))
        csd_pos_gamma = csd_morlet(pos, frequencies=freqs["gamma"], n_jobs=8, n_cycles=cycles["gamma"], decim=1)

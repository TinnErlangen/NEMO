...
for freq,vals in freqs.items():
    ...
    contrast.save('{dir}NEMO_neg_vs_ton_contrast_{f}-avg-connectivity.h5'.format(dir=meg_dir,f=freq))

    # Perform a permutation test to only retain connections that are part of a significant bundle.
    stats = conpy.cluster_permutation_test(cons['neg'], cons['ton'],cluster_threshold=5, src=fs_src, n_permutations=1000, verbose=True,
    ...
    write_hdf5('{dir}NEMO_neg_vs_ton_contrast_{f}-stats.h5'.format(dir=meg_dir,f=freq),dict(connection_indices=connection_indices,bundles=bundles,bundle_ts=bundle_ts,bundle_ps=bundle_ps,H0=H0),overwrite=True)

    # Save the pruned grand average connection object
    con_clust.save('{dir}NEMO_neg_vs_ton_contrast_{f}-pruned-avg-connectivity.h5'.format(dir=meg_dir,f=freq))
    ...
    con_parc.save('{dir}NEMO_neg_vs_ton_contrast_{f}-pruned-label-avg-connectivity.h5'.format(dir=meg_dir,f=freq))
    # print them out

...
save_dir = "D:/NEMO_analyses/plots/connectivity_new/"
...
con = conpy.read_connectivity('{dir}NEMO_neg_vs_ton_contrast_{f}-pruned-avg-connectivity.h5'.format(dir=meg_dir,f=freq))
con_parc = conpy.read_connectivity('{dir}NEMO_neg_vs_ton_contrast_{f}-pruned-label-avg-connectivity.h5'.format(dir=meg_dir,f=freq))
...
brain.add_annotation('aparc.a2009s', borders=1)

# Save some views
mlab.view(0, 90, 450, [0, 0, 0])
mlab.savefig('{d}neg-ton_connectivity_{f}_degree_rh.png'.format(d=save_dir,f=freq), magnification=4)
mlab.view(180, 90, 450, [0, 0, 0])
mlab.savefig('{d}neg-ton_connectivity_{f}_degree_lh.png'.format(d=save_dir,f=freq), magnification=4)
mlab.view(180, 0, 450, [0, 10, 0])
mlab.savefig('{d}neg-ton_connectivity_{f}_degree_top.png'.format(d=save_dir,f=freq), magnification=4)
mlab.view(180, 180, 480, [0, 10, 0])
mlab.savefig('{d}neg-ton_connectivity_{f}_degree_bottom.png'.format(d=save_dir,f=freq), magnification=4)
...
fig.savefig('{d}neg-ton_connectivity_{f}_squircle.pdf'.format(d=save_dir,f=freq), bbox_inches='tight')
